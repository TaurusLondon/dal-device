...
public class BeanTest {
	...
	public static <T> void test(final Class<T> clazz) throws IntrospectionException, NoSuchMethodException {
		test(clazz, "class");
	}

	public static <T> void test(final Class<T> clazz, final String... skips) throws IntrospectionException {
			...
			if (getter != null && setter != null) {
				...
				if (paramsClassArray.length == 1 && paramsClassArray[0] == returnClass) {
					try {
						...
						assertEquals(String.format("Failed while testing property %s", propertyName), expectedValue,
								actualValue);
						...
						fail(String.format("An exception was thrown while testing the property %s: %s", propertyName,
								ex.toString()));
					}
				}
			}
		...
		try {
			...
			fail(String.format("An exception was thrown while testing the property %s: %s", e.toString()));
			e.printStackTrace();
		}
	}

	private static <T> Object buildValue(Class<T> clazz)
		...
		if (!Modifier.isFinal(clazz.getModifiers())) {
			...
			if (clazz == String.class) {
				...
				fail("Unable to build an instance of class " + clazz.getName() + ", please add some code to the "
						+ BeanTest.class.getName() + " class to do this.");
				return null;
			}
		}
	}
